...
void node_state_machine_init(void) {
  ...
  g_node_fsm.state_next =
      is_wake_from_deep_sleep ? NODE_STATE_MEASURE : NODE_STATE_IDLE;
  ...
  ESP_LOGI(TAG,
           "Khởi tạo Node State Machine THÀNH CÔNG! (Lý do boot: %s | Chu kỳ: "
           "%lu ms)",
  ...
  while (1) {
    if (ota_node_is_updating()) {
      ...
      if (last_act > 0 && (now_ms - last_act > 30000)) {
        ESP_LOGE(TAG, "[OTA TIMEOUT] Quá 30 giây không nhận được dữ liệu từ "
                      "Master! Tự động hủy OTA và khôi phục đo đạc...");
        ota_node_abort();
        continue;
      }
    ...
    */
    case NODE_STATE_MEASURE: {
      ...
      if (err == ESP_OK) {
        ...
        // Nếu nước đứng yên (chênh lệch < 0.8cm): giãn chu kỳ gửi lên 5 giây
        // (tiết kiệm 60% pin) Nếu nước đang thay đổi (đang bơm / xả chênh lệch
        // >= 0.8cm): đo nhanh 2 giây để đóng ngắt chuẩn
      ...
      // Đọc điện áp pin thực tế từ phần cứng qua ADC
      float bat_volt = 0.0f;
      if (battery_read_voltage(&bat_volt) == ESP_OK) {
        g_node_fsm.current_battery_volt = bat_volt;
        ESP_LOGI(TAG, "🔋 Điện áp pin thực tế: %.2fV", bat_volt);
      } else {
        ESP_LOGW(TAG, "⚠️ Không đọc được ADC pin, giữ giá trị: %.2fV",
                 g_node_fsm.current_battery_volt);
      }
    ...
    */
    case NODE_STATE_SEND_DATA: {
      ...
      strncpy(packet.fw_version, ota_node_get_version(),
              sizeof(packet.fw_version) - 1);
      ...
      // CỬA SỔ ĐÓN TIN OTA TỪ MASTER: Lắng nghe 80ms xem Master có gửi lệnh OTA
      // không
      bool has_ota = esp_now_node_wait_for_ota_trigger(80);
      if (has_ota || ota_node_is_updating()) {
        ESP_LOGW(TAG, "🚀 [OTA PHÁT HIỆN] Master đang gửi lệnh OTA! Hủy Deep "
                      "Sleep và chuẩn bị nhận firmware...");
        node_state_machine_set_state(NODE_STATE_OTA_UPDATING);
        break;
      }

      // Nếu Master chưa khóa kênh (lần đầu boot): chạy bình thường không ngủ để
      // bắt tay
    ...
    case NODE_STATE_SLEEP: {
      ...
      ESP_LOGI(TAG,
               "💤 [SMART DEEP SLEEP] Ngủ sâu %lu ms (Dòng ~15µA - Tiết kiệm "
               ">94%% pin)...",
    ...
    case NODE_STATE_OTA_UPDATING: {
      if (!ota_node_is_updating()) {
        ESP_LOGI(TAG, "Phiên OTA đã kết thúc hoặc bị hủy -> Quay lại trạng "
                      "thái IDLE đo đạc...");
      ...
      if (last_active > 0 &&
          (esp_timer_get_time() / 1000 - last_active) > 15000) {
        ESP_LOGE(TAG, "Quá 15 giây không nhận được dữ liệu OTA mới -> Hủy "
                      "phiên OTA và quay về đo đạc!");
        ota_node_abort();
        break;
      }

      ESP_LOGW(TAG, "===> ĐANG NẠP OTA QUA ESP-NOW (TẠM DỪNG ĐO VÀ KHÔNG BẮN "
                    "DỮ LIỆU)...");
      vTaskDelay(pdMS_TO_TICKS(1000));
      break;
    }
