    ...
    : QDialog(parent)
{
    ...
    auto *titleLbl = new QLabel(QStringLiteral("Chọn Thiết Bị Online"));
...
void SelectOnlineDeviceDialog::populateDeviceList(const QJsonArray &devices)
{
    ...
    for (const auto &val : devices) {
        ...
        // Filter for Son devices: son-190782 or 150304
        if (devId != QStringLiteral("son-190782") && devId != QStringLiteral("150304") && !devId.startsWith(QStringLiteral("son-"))) {
            continue;
        }
