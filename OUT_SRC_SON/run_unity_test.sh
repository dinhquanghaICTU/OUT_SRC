...
elif [ -d "$SCRIPT_DIR/OUT_SRC_SON/unity_test" ]; then
    TEST_DIR="$SCRIPT_DIR/OUT_SRC_SON/unity_test"
elif [ -d "$(pwd)/unity_test" ]; then
    TEST_DIR="$(pwd)/unity_test"
else
    echo -e "\033[1;31mError: Cannot find unity_test directory for OUT_SRC_SON!\033[0m"
    exit 1
fi

echo -e "\033[1;34m============================================================\033[0m"
echo -e "\033[1;34m  BUILDING & RUNNING UNITY TEST: Son (HC-SR04 + Water Pump & Flow)\033[0m"
echo -e "\033[1;34m  Project: OUT_SRC_SON\033[0m"
...
gcc -Wall -Wextra -O2 -I. -o run_test_bin unity.c mock_hal.c test_son_firmware.c -lm
...
if [ $TEST_RESULT -eq 0 ]; then
    echo -e "\033[1;32m✓ [OUT_SRC_SON] ALL UNITY TESTS PASSED SUCCESSFULLY!\033[0m\n"
else
    echo -e "\033[1;31m✗ [OUT_SRC_SON] UNITY TESTS FAILED (code $TEST_RESULT)!\033[0m\n"
    exit $TEST_RESULT
fi
