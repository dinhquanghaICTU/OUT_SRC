...
bool Database::seedDefaults(QString *error)
{
    ...
    cleanup.exec(QStringLiteral("DELETE FROM discovered_devices WHERE device_id != '150808' AND device_id != 'Tuananh-150808' AND device_id != 'Tuananh-150304'"));
    cleanup.exec(QStringLiteral("DELETE FROM devices WHERE device_id != '150808' AND device_id != 'Tuananh-150808' AND device_id != 'Tuananh-150304'"));
    cleanup.exec(QStringLiteral("DELETE FROM device_telemetry_log WHERE device_id != '150808' AND device_id != 'Tuananh-150808' AND device_id != 'Tuananh-150304'"));
    ...
    if (checkLoginHist.exec(QStringLiteral("SELECT COUNT(*) FROM login_history")) && checkLoginHist.next()) {
        if (checkLoginHist.value(0).toInt() == 0) {
            ...
            q.exec(QStringLiteral("INSERT INTO login_history(username,role,ip_address,status,created_at) VALUES('tuananh','user','192.168.1.114','success','%1')").arg(t2));
            q.exec(QStringLiteral("INSERT INTO login_history(username,role,ip_address,status,created_at) VALUES('guest','user','192.168.1.120','failed','%1')").arg(t3));
        }
    }
    ...
    if (checkAudit.exec(QStringLiteral("SELECT COUNT(*) FROM audit_log")) && checkAudit.next()) {
        if (checkAudit.value(0).toInt() == 0) {
            ...
            QSqlQuery q(m_db);
            q.exec(QStringLiteral("INSERT INTO audit_log(username,role,action,target,details,created_at) VALUES('admin','admin','ĐIỀU KHIỂN RƠ-LE','Tuananh-150808','Bật rơ-le thiết bị','%1')").arg(t1));
            q.exec(QStringLiteral("INSERT INTO audit_log(username,role,action,target,details,created_at) VALUES('tuananh','user','CÀI ĐẶT CẤU HÌNH','Tuananh-150808','Cập nhật thông số ngưỡng hệ thống','%1')").arg(t2));
            q.exec(QStringLiteral("INSERT INTO audit_log(username,role,action,target,details,created_at) VALUES('admin','admin','GHÉP NỐI THIẾT BỊ','Tuananh-150808','Gán trạm đo Tuấn Anh','%1')").arg(t3));
        }
    }
                                    ...
                                    const QJsonObject &metrics, QString *error)
{
    ...
    if (normalizedId.compare(QStringLiteral("150808"), Qt::CaseInsensitive) != 0 &&
        ...
        normalizedId.compare(QStringLiteral("Tuananh-150304"), Qt::CaseInsensitive) != 0)
        return false;
    const QString now = QDateTime::currentDateTimeUtc().toString(Qt::ISODateWithMs);
...
QJsonArray Database::availableDevices(int onlineWindowSeconds, QString *error) const
{
    ...
    query.prepare(QStringLiteral(
        ...
        "WHERE d.online=1 AND d.last_seen_at>=? AND (d.device_id='150808' OR d.device_id='Tuananh-150808' OR d.device_id='Tuananh-150304') "
