...
void begin_fixed_station(void)
{
    ...
    Serial.print("[WiFi] Ket noi WiFi co dinh: ");
...
void wifi_manager_update(void)
{
    ...
    if (manager_state == WIFI_MANAGER_CONNECTED)
    {
        Serial.println("[WiFi] Mat ket noi Pi AP");
        printed_connected = false;
    }
...
{
    // Đã bỏ chế độ ESP phát AP cấu hình. Hàm giữ lại để code cũ không lỗi compile.
    Serial.println("[WiFi] Fixed mode: khong con che do AP config, reconnect Pi AP");
    begin_fixed_station();
}
...
{
    // Không dùng Preferences nữa vì WiFi/MQTT đã cố định trong config.h.
    Serial.println("[WiFi] Fixed mode: khong co cau hinh flash de xoa");
    begin_fixed_station();
    return true;
}
