...
bool Database::seedDefaults(QString *error)
{
    ...
    if (checkLoginHist.exec(QStringLiteral("SELECT COUNT(*) FROM login_history")) && checkLoginHist.next()) {
        if (checkLoginHist.value(0).toInt() == 0) {
            ...
            q.exec(QStringLiteral("INSERT INTO login_history(username,role,ip_address,status,created_at) VALUES('lenam','user','192.168.1.115','success','%1')").arg(t2));
            q.exec(QStringLiteral("INSERT INTO login_history(username,role,ip_address,status,created_at) VALUES('guest','user','192.168.1.122','failed','%1')").arg(t3));
        }
    }
    ...
    if (checkAudit.exec(QStringLiteral("SELECT COUNT(*) FROM audit_log")) && checkAudit.next()) {
        if (checkAudit.value(0).toInt() == 0) {
            ...
            QSqlQuery q(m_db);
            q.exec(QStringLiteral("INSERT INTO audit_log(username,role,action,target,details,created_at) VALUES('admin','admin','ĐIỀU KHIỂN RƠ-LE','HA-190782','Bật thiết bị khí quyển','%1')").arg(t1));
            q.exec(QStringLiteral("INSERT INTO audit_log(username,role,action,target,details,created_at) VALUES('lenam','user','CÀI ĐẶT CẤU HÌNH','HA-190782','Cập nhật ngưỡng áp suất & nhiệt độ','%1')").arg(t2));
            q.exec(QStringLiteral("INSERT INTO audit_log(username,role,action,target,details,created_at) VALUES('admin','admin','GHÉP NỐI THIẾT BỊ','HA-190782','Gán trạm khí quyển số 1','%1')").arg(t3));
        }
    }
