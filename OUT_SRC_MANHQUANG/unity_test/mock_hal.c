...
void mock_hal_set_time_ms(uint64_t ms) { s_virtual_time_ms = ms; }
void mock_hal_advance_time_ms(uint64_t ms) { s_virtual_time_ms += ms; }
uint64_t mock_hal_get_time_ms(void) { return s_virtual_time_ms; }
uint64_t mock_hal_get_time_us(void) { return s_virtual_time_ms * 1000ULL; }

void mock_gpio_set_mode(int pin, mock_gpio_mode_t mode) { if (pin >= 0 && pin < MOCK_MAX_PINS) s_pin_modes[pin] = mode; }
void mock_gpio_write(int pin, int level) { if (pin >= 0 && pin < MOCK_MAX_PINS) s_pin_outputs[pin] = level ? 1 : 0; }
void mock_gpio_set_input_level(int pin, int level) { if (pin >= 0 && pin < MOCK_MAX_PINS) s_pin_inputs[pin] = level ? 1 : 0; }
int mock_gpio_read(int pin) {
    if (pin >= 0 && pin < MOCK_MAX_PINS) {
        if (s_pin_modes[pin] == MOCK_GPIO_MODE_OUTPUT) return s_pin_outputs[pin];
        return s_pin_inputs[pin];
    }
    return 0;
}
int mock_gpio_get_output_level(int pin) { return (pin >= 0 && pin < MOCK_MAX_PINS) ? s_pin_outputs[pin] : 0; }
...
bool json_extract_string(const char *json, const char *key, char *out_val, size_t max_len) {
    ...
    p++;
    size_t i = 0;
    while (*p && *p != '"' && i < max_len - 1) out_val[i++] = *p++;
    out_val[i] = '\0';
    return true;
}
