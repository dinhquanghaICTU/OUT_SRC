...
void ApiClient::requestLatestReading()
{
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply] {
        ...
        SensorReading r;
        r.doorPositionPct = obj.value(QStringLiteral("door_position_pct")).toDouble();
        r.motionDetected = obj.value(QStringLiteral("motion_detected")).toBool();
        r.irBlocked = obj.value(QStringLiteral("ir_blocked")).toBool();
        r.motorSpeedRpm = obj.value(QStringLiteral("motor_speed_rpm")).toDouble();
        r.passageCount = obj.value(QStringLiteral("passage_count")).toInt();
        r.temperatureC = obj.value(QStringLiteral("temperature_c")).toDouble(28.0);
...
{
    QJsonObject obj{
        {"motor_speed_steps", config.motorSpeedStepsPerSec},
        {"max_travel_steps", config.maxTravelSteps},
        {"auto_close_delay_s", config.autoCloseDelaySeconds},
        {"microstepping", config.microsteppingMode},
...
void ApiClient::sendDoorCommand(const QString &deviceId, const QString &action)
{
    // If action is "open", state = true; "close", state = false
    setRelayState(deviceId, action == QStringLiteral("open"));
}
