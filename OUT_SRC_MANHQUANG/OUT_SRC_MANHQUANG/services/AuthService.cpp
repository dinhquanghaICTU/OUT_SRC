    ...
    : QObject(parent), m_apiClient(apiClient)
{
    ...
    connect(m_apiClient, &ApiClient::loginFailed, this, [this](const QString &message) {
        const bool offlineAdmin = (m_pendingUsername.trimmed() == QStringLiteral("admin") && m_pendingPassword == QStringLiteral("admin123"))
            || (m_pendingUsername.trimmed() == QStringLiteral("admin1") && m_pendingPassword == QStringLiteral("1"));
        ...
        if (offlineAdmin && !serverRejectedCredentials) {
            m_currentUser.username = m_pendingUsername.trimmed();
...
void AuthService::login(const QString &username, const QString &password)
{
    ...
    if (AppConfig::DemoMode) {
        if ((username.trimmed() == QStringLiteral("admin") && password == QStringLiteral("1")) ||
            (username.trimmed() == QStringLiteral("admin") && password == QStringLiteral("admin123"))) {
            m_currentUser.role = QStringLiteral("admin");
            m_authenticated = true;
            m_offlineMode = false;
            emit authenticated();
            ...
            emit authenticationFailed(
                tr("Tài khoản hoặc mật khẩu không đúng. Dùng admin / admin123 để xem demo."));
        }
        return;
    }
...
{
    return m_currentUser.role.isEmpty() ? QStringLiteral("user") : m_currentUser.role;
}
