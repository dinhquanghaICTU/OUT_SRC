...
static bool parse_json_string(const char *payload, const char *field,
...
static void publish_command_result(const char *command_id, bool state) {
  ...
  const int length = snprintf(payload, sizeof(payload),
                              ...
                              "\"status\":\"succeeded\","
                              "\"state\":{\"relay\":%s}}",
                              command_id, state ? "true" : "false");

  if (length > 0 && length < (int)sizeof(payload))
    esp_mqtt_client_publish(mqtt_client, MQTT_COMMAND_RESULT_TOPIC, payload,
                            length, 1, 0);
}
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base,
  ...
  case MQTT_EVENT_DATA: {
    ...
    // 1. Check if Config Desired topic
    if (event->topic_len == (int)strlen(MQTT_CONFIG_DESIRED_TOPIC) &&
        memcmp(event->topic, MQTT_CONFIG_DESIRED_TOPIC, event->topic_len) ==
            0) {
      char payload[256];
      ...
      if (strstr(payload, "\"auto_mode\":true") != NULL ||
          strstr(payload, "\"auto_mode\": true") != NULL) {
        auto_mode = true;
      }
      ...
      if (!pStart)
        pStart = strstr(payload, "\"distance_start\":");
      if (pStart) {
        char *colon = strchr(pStart, ':');
        if (colon)
          start_cm = (float)atof(colon + 1);
      }

      char *pStop = strstr(payload, "\"distance_stop_cm\":");
      if (!pStop)
        pStop = strstr(payload, "\"distance_stop\":");
      if (pStop) {
        char *colon = strchr(pStop, ':');
        if (colon)
          stop_cm = (float)atof(colon + 1);
      }
    ...
    // 2. Check if Relay Command topic
    ...
    if (!parse_json_string(payload, "command_id", command_id,
                           sizeof(command_id)) ||
        !parse_json_string(payload, "type", command_type,
                           sizeof(command_type)) ||
...
bool mqtt_manager_is_connected(void) { return mqtt_connected; }

bool mqtt_manager_publish_sensor(float temperature, float humidity,
                                 float soil_moisture) {
  char payload[384];
  int length;
  ...
  const int message_id = esp_mqtt_client_publish(
      mqtt_client, MQTT_TELEMETRY_TOPIC, payload, length, 1, 0);
...
bool mqtt_manager_publish_relay(bool state, const char *changed_by) {
  ...
  length = snprintf(payload, sizeof(payload),
                    ...
                    "\"changed_by\":\"%s\","
                    "\"uptime_ms\":%" PRIu64 "}",
                    state ? "true" : "false", changed_by,
                    (uint64_t)(esp_timer_get_time() / 1000));

  if (length <= 0 || length >= (int)sizeof(payload))
    return false;

  return esp_mqtt_client_publish(mqtt_client, MQTT_STATE_TOPIC, payload, length,
                                 1, 1) >= 0;
}
