...
bool EmbeddedServer::start(const Config &config, QString *error)
{
    ...
    if (m_databasePath.isEmpty()) {
        ...
        m_databasePath = dataDir + QStringLiteral("/irrigation_system.db");
    }
    ...
    qInfo().noquote() << QStringLiteral("Van Phong Smart Irrigation Server listening on %1").arg(baseUrl());
    qInfo().noquote() << QStringLiteral("SQLite: %1").arg(m_databasePath);
    qInfo().noquote() << QStringLiteral("MQTT: %1:%2").arg(m_config.mqttHost).arg(m_config.mqttPort);
    return true;
}
