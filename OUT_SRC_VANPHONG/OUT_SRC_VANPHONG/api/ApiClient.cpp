...
QString ApiClient::responseError(const QByteArray &body,
...
void ApiClient::handleNetworkError(const QString &operation,
                                   QNetworkReply *reply) {
...
void ApiClient::login(const QString &username, const QString &password) {
  auto *reply = m_networkManager.post(
      ...
      QJsonDocument(QJsonObject{{"username", username}, {"password", password}})
          .toJson());
  ...
  connect(reply, &QNetworkReply::finished, this, [this, reply] {
    ...
    if (reply->error() != QNetworkReply::NoError) {
      emit loginFailed(
          responseError(body, tr("Tài khoản hoặc mật khẩu không đúng")));
      return;
    }
    const QJsonObject root = QJsonDocument::fromJson(body).object();
    m_accessToken = root.value(QStringLiteral("token")).toString();
    const QString role = root.value(QStringLiteral("user"))
                             .toObject()
...
void ApiClient::requestLatestReading() {
  ...
  connect(reply, &QNetworkReply::finished, this, [this, reply] {
    ...
    SensorReading r;
    r.soilMoisturePct =
        obj.value(QStringLiteral("soil_moisture")).toDouble(55.0);
    r.temperatureC =
        obj.value(QStringLiteral("temperature_c"))
            .toDouble(obj.value(QStringLiteral("temperature")).toDouble(27.5));
    r.humidityPct =
        obj.value(QStringLiteral("humidity"))
            .toDouble(obj.value(QStringLiteral("humidity_pct")).toDouble(65.0));
    r.pumpActive =
        obj.value(QStringLiteral("pump_active"))
            .toBool(obj.value(QStringLiteral("relay")).toBool(false));
    r.waterTankLevelPct =
        obj.value(QStringLiteral("water_tank_level")).toDouble(85.0);
...
void ApiClient::requestAlerts() {
  auto *reply =
      m_networkManager.get(makeRequest(QStringLiteral("/api/alerts")));
  connect(reply, &QNetworkReply::finished, reply, &QObject::deleteLater);
}

void ApiClient::updateDeviceConfig(const DeviceConfig &config) {
  QJsonObject obj{{"min_soil_moisture", config.minSoilMoistureTriggerPct},
  ...
  auto *reply = m_networkManager.put(makeRequest(QStringLiteral("/api/config")),
...
void ApiClient::requestMyDevice() {
  ...
  auto *reply =
      m_networkManager.get(makeRequest(QStringLiteral("/api/devices/me")));
  connect(reply, &QNetworkReply::finished, this, [this, reply] {
    reply->deleteLater();
...
void ApiClient::claimDevice(const QString &deviceId, const QString &name) {
  auto *reply = m_networkManager.post(
      ...
      QJsonDocument(QJsonObject{{"device_id", deviceId}, {"name", name}})
          .toJson());
  ...
  connect(reply, &QNetworkReply::finished, this, [this, deviceId, reply] {
    ...
    if (reply->error() != QNetworkReply::NoError) {
      emit operationFailed(
          responseError(reply->readAll(), tr("Không thể gỡ thiết bị")));
      return;
    }
...
void ApiClient::createUser(const QString &username, const QString &password,
                           const QString &role) {
  auto *reply =
      m_networkManager.post(makeRequest(QStringLiteral("/api/users")),
                            QJsonDocument(QJsonObject{{"username", username},
                                                      {"password", password},
                                                      {"role", role}})
                                .toJson());
  ...
  connect(reply, &QNetworkReply::finished, this, [this, reply] {
    ...
    if (reply->error() != QNetworkReply::NoError) {
      emit operationFailed(
          responseError(reply->readAll(), tr("Không thể tạo người dùng")));
      return;
    }
    emit userCreated();
  });
...
void ApiClient::updateUser(const QString &oldUsername, const QString &username,
  ...
  connect(reply, &QNetworkReply::finished, this, [this, reply] {
    ...
    if (reply->error() != QNetworkReply::NoError) {
      emit operationFailed(
          responseError(reply->readAll(), tr("Không thể cập nhật người dùng")));
      return;
    }
    emit userUpdated();
  });
...
void ApiClient::deleteUser(const QString &username) {
  auto *reply = m_networkManager.deleteResource(
      makeRequest(QStringLiteral("/api/users/%1").arg(username)));
  connect(reply, &QNetworkReply::finished, this, [this, reply] {
    reply->deleteLater();
    if (reply->error() != QNetworkReply::NoError) {
      emit operationFailed(
          responseError(reply->readAll(), tr("Không thể xóa người dùng")));
      return;
    }
    emit userDeleted();
  });
...
void ApiClient::releaseUserDevice(const QString &username,
...
void ApiClient::setRelayState(const QString &deviceId, bool state) {
  ...
  connect(reply, &QNetworkReply::finished, this, [this, deviceId, reply] {
    ...
    if (reply->error() != QNetworkReply::NoError) {
      emit operationFailed(
          responseError(reply->readAll(), tr("Không thể gửi lệnh")));
      return;
    }
...
void ApiClient::updatePerDeviceConfig(const QString &deviceId,
  ...
  connect(reply, &QNetworkReply::finished, this, [this, deviceId, reply] {
    ...
    emit deviceConfigSaved(
        deviceId, res.value(QStringLiteral("mqtt_published")).toBool());
  });
}

void ApiClient::requestDeviceHistory(const QString &deviceId,
                                     const QString &period,
                                     const QString &date) {
  QUrl url(urlFor(QStringLiteral("/api/devices/%1/history").arg(deviceId)));
  QUrlQuery q;
  if (!period.isEmpty())
    q.addQueryItem(QStringLiteral("period"), period);
  if (!date.isEmpty())
