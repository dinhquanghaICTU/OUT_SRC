...
void MqttDiscoveryService::stop()
{
    m_stopping = true;
...
bool MqttDiscoveryService::isConnected() const
{
...
bool MqttDiscoveryService::publishPumpCommand(const QString &deviceId, const QString &commandId,
                                              bool state, int durationSeconds)
{
    if (m_socket.state() != QAbstractSocket::ConnectedState)
        return false;
    const QByteArray topic = QByteArrayLiteral("iot/v1/devices/")
    ...
    QJsonObject params{{"state", state}};
    if (durationSeconds > 0)
        params.insert(QStringLiteral("duration_s"), durationSeconds);
...
void MqttDiscoveryService::sendConnect()
{
    ...
    const QByteArray clientId = QByteArrayLiteral("vanphong-server-")
