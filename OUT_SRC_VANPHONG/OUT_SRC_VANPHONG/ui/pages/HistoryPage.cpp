...
static QColor colorForMetric(const QString &key) {
  if (key == QStringLiteral("soil_moisture"))
    return QColor(QStringLiteral("#10b981")); // Emerald
  if (key == QStringLiteral("temperature_c") ||
      key == QStringLiteral("temperature"))
    return QColor(QStringLiteral("#f59e0b")); // Amber
  if (key == QStringLiteral("humidity") ||
      key == QStringLiteral("humidity_pct"))
    return QColor(QStringLiteral("#0284c7")); // Sky
  if (key == QStringLiteral("water_tank_level"))
    return QColor(QStringLiteral("#06b6d4")); // Cyan
...
HistoryPage::HistoryPage(QWidget *parent)
  ...
  ui->metricCombo->addItem(tr("🌱 Độ ẩm đất (%)"),
                           QStringLiteral("soil_moisture"));
  ui->metricCombo->addItem(tr("🌡️ Nhiệt độ khí (°C)"),
                           QStringLiteral("temperature_c"));
  ui->metricCombo->addItem(tr("💧 Độ ẩm không khí (%)"),
                           QStringLiteral("humidity"));
  ui->metricCombo->addItem(tr("🌊 Mức bồn nước (%)"),
  ...
  ui->historyTable->setHorizontalHeaderLabels(
      {tr("Thời gian"), tr("Sự kiện"), tr("Độ ẩm đất (%)"), tr("Nhiệt độ (°C)"),
       tr("Độ ẩm khí (%RH)"), tr("Mức bồn nước")});
  ...
  connect(ui->deviceCombo, QOverload<int>::of(&QComboBox::currentIndexChanged),
          this, [this](int idx) {
            if (idx >= 0 && idx < m_devices.size()) {
              m_selectedDeviceId = m_devices.at(idx)
                                       .toObject()
  ...
  connect(ui->dateCombo, QOverload<int>::of(&QComboBox::currentIndexChanged),
          this, [this](int idx) {
            if (idx >= 0) {
...
void HistoryPage::rebuildDateOptions() {
  ...
  // Hàm kiểm tra xem 1 date có trong danh sách có data không
  // Nếu m_availableDates rỗng (chưa có data từ server) -> hiện tất cả
  auto hasData = [&](const QDate &d) -> bool {
    if (m_availableDates.isEmpty())
      return true;
    if (period == QStringLiteral("year")) {
      return m_availableDates.contains(d.toString(QStringLiteral("yyyy")));
    } else if (period == QStringLiteral("month")) {
      return m_availableDates.contains(d.toString(QStringLiteral("yyyy-MM")));
    } else {
      return m_availableDates.contains(d.toString(Qt::ISODate));
    }
  };
  ...
  // Nếu không có ngày nào có dữ liệu, hiện placeholder
  if (ui->dateCombo->count() == 0) {
    ui->dateCombo->addItem(tr("Không có lịch sử"), QDate());
...
void HistoryPage::setDevices(const QJsonArray &devices) {
  ...
  if (!devices.isEmpty()) {
    m_selectedDeviceId = devices.first()
                             .toObject()
                             .value(QStringLiteral("device_id"))
                             .toString();
    requestCurrentHistory();
...
void HistoryPage::requestCurrentHistory() {
  ...
  const QString period = ui->periodCombo->currentData().toString();
  const QString dateStr = m_selectedDate.isValid()
...
void HistoryPage::setHistory(const QJsonObject &history) {
  ...
  // Cập nhật danh sách ngày có dữ liệu từ server
  // Server có thể trả về "available_dates" là mảng các date string
  const QJsonArray availArr =
      history.value(QStringLiteral("available_dates")).toArray();
  if (!availArr.isEmpty()) {
    m_availableDates.clear();
    for (const auto &v : availArr) {
      m_availableDates.insert(v.toString());
    }
    rebuildDateOptions();
  }
  ...
  // Stat Cards
  const int waterCount = history.value(QStringLiteral("water_count")).toInt(0);
  const double waterVol =
      history.value(QStringLiteral("water_volume")).toDouble(0.0);
  const QJsonObject avgs = history.value(QStringLiteral("averages")).toObject();
  const double avgSoil =
      avgs.value(QStringLiteral("soil_moisture")).toDouble(0.0);
  const double maxTemp =
      history.value(QStringLiteral("max_temperature")).toDouble(0.0);

  ui->valWaterCount->setText(tr("%1 lần").arg(waterCount));
  ui->valWaterVolume->setText(
      tr("%1 Lít").arg(QString::number(waterVol, 'f', 1)));
  ui->valAvgSoil->setText(tr("%1 %").arg(QString::number(avgSoil, 'f', 1)));
  ui->valMaxTemp->setText(tr("%1 °C").arg(QString::number(maxTemp, 'f', 1)));

  // Dynamic header based on period (Thời gian (Phút) cho Ngày, Ngày cho Tháng,
  // Tháng cho Năm)
  const QString currentPeriod =
      history.value(QStringLiteral("period"))
          .toString(ui->periodCombo->currentData().toString());
  ...
  ui->historyTable->setHorizontalHeaderLabels(
      {timeColHeader, tr("Lượt tưới"), tr("Độ ẩm đất (TB %)"),
       tr("Nhiệt độ (TB °C)"), tr("Độ ẩm khí (TB %RH)"),
       tr("Mức bồn nước (TB)")});
  ...
  for (const auto &val : rows) {
    ...
    const double sm = row.value(QStringLiteral("soil_moisture")).toDouble(0.0);
    const double t = row.value(QStringLiteral("temperature_c")).toDouble(0.0);
    const double h = row.value(QStringLiteral("humidity")).toDouble(0.0);
    const double tank =
        row.value(QStringLiteral("water_tank_level")).toDouble(0.0);

    ui->historyTable->setItem(r, 0, new QTableWidgetItem(label));
    ui->historyTable->setItem(
        r, 1,
        new QTableWidgetItem(pumpCount > 0 ? tr("💦 %1 lần tưới").arg(pumpCount)
                                           : tr("🌱 Giám sát ổn định")));
    ui->historyTable->setItem(r, 2,
                              new QTableWidgetItem(QStringLiteral("%1%").arg(
                                  QString::number(sm, 'f', 1))));
    ui->historyTable->setItem(r, 3,
                              new QTableWidgetItem(QStringLiteral("%1 °C").arg(
                                  QString::number(t, 'f', 1))));
    ui->historyTable->setItem(r, 4,
                              new QTableWidgetItem(QStringLiteral("%1 %RH").arg(
                                  QString::number(h, 'f', 1))));
    ui->historyTable->setItem(
        r, 5,
        new QTableWidgetItem(
            tank > 0.0
                ? tr("Bình thường (%1%)").arg(QString::number(tank, 'f', 0))
                : tr("--")));
  }
...
void HistoryPage::updateChart() {
  ...
  const QJsonArray dataArray =
      m_cachedHistory.value(QStringLiteral("data")).toArray();
  ...
  for (const auto &item : chartPoints) {
    ...
    // Bỏ qua ngày không có dữ liệu thực (key không tồn tại trong JSON)
    if (!obj.contains(m_selectedMetricKey))
      continue;
  ...
  // Nếu sau khi lọc không có điểm dữ liệu nào, xóa series và thoát
...
QString HistoryPage::metricTitle(const QString &key) {
  ...
  if (key == QStringLiteral("temperature_c") ||
      key == QStringLiteral("temperature"))
    return tr("Nhiệt độ khí (°C)");
  if (key == QStringLiteral("humidity") ||
      key == QStringLiteral("humidity_pct"))
    return tr("Độ ẩm không khí (%RH)");
  if (key == QStringLiteral("water_tank_level"))
    return tr("Mức bồn nước (%)");
  return key;
}
