      ...
      m_userManagementPage(new UserManagementPage(this))
{
    ...
    const bool savedSidebarExpanded = QSettings().value(
        QStringLiteral("ui/sidebarExpanded"), false).toBool();
...
void MainWindow::resizeEvent(QResizeEvent *event)
{
    ...
    if (event->size().width() < 1020 && m_sidebarExpanded)
...
void MainWindow::setSidebarExpanded(bool expanded)
{
    ...
    const int width = expanded ? 196 : 66;
...
{
    auto setNavText = [this](QPushButton *button, const QString &icon, const QString &label) {
