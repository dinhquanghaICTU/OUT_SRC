...
bool Database::seedDefaults(QString *error)
{
    ...
    user.prepare(QStringLiteral(
        "INSERT OR IGNORE INTO users(id,username,password_hash,password_salt,role,created_at) "
    ...
    // Ensure device 190782 (Hoang Minh Automatic Cooling System) is registered
    ...
    devQuery.prepare(QStringLiteral(
        ...
        "VALUES('190782','Trạm Làm Mát Tự Động',1,?)"));
    devQuery.addBindValue(now);
    devQuery.exec();

    QSqlQuery discQuery(m_db);
    discQuery.prepare(QStringLiteral(
        "INSERT OR REPLACE INTO discovered_devices(device_id,online,device_type,metrics_json,first_seen_at,last_seen_at) "
        "VALUES('190782',1,'temperature_sound','{\"temperature_c\":28.5,\"sound_vpp\":0.12,\"relay_state\":false}',?,?)"));
    discQuery.addBindValue(now);
    discQuery.addBindValue(now);
    discQuery.exec();

    // Clean up old unrelated telemetry
    QSqlQuery cleanOld(m_db);
    cleanOld.exec(QStringLiteral("DELETE FROM device_telemetry_log WHERE metrics_json LIKE '%pressure_hpa%' OR metrics_json LIKE '%distance_cm%'"));
    cleanOld.exec(QStringLiteral("DELETE FROM device_telemetry_log WHERE coalesce(substr(datetime(recorded_at, 'localtime'), 1, 10), substr(recorded_at, 1, 10)) > date('now', 'localtime')"));
    cleanOld.exec(QStringLiteral("DELETE FROM device_telemetry_log WHERE json_extract(metrics_json, '$.temperature_c') > 65.0 OR json_extract(metrics_json, '$.temperature_c') < 5.0"));
    // Delete any old generated fake data from days before today as user requested
    cleanOld.exec(QStringLiteral("DELETE FROM device_telemetry_log WHERE coalesce(substr(datetime(recorded_at, 'localtime'), 1, 10), substr(recorded_at, 1, 10)) < date('now', 'localtime')"));
    ...
    if (checkLoginHist.exec(QStringLiteral("SELECT COUNT(*) FROM login_history")) && checkLoginHist.next()) {
        if (checkLoginHist.value(0).toInt() == 0) {
            ...
            q.exec(QStringLiteral("INSERT INTO login_history(username,role,ip_address,status,created_at) VALUES('hoangminh','user','192.168.1.112','success','%1')").arg(t2));
            q.exec(QStringLiteral("INSERT INTO login_history(username,role,ip_address,status,created_at) VALUES('guest','user','192.168.1.118','failed','%1')").arg(t3));
        }
    }
    ...
    if (checkAudit.exec(QStringLiteral("SELECT COUNT(*) FROM audit_log")) && checkAudit.next()) {
        if (checkAudit.value(0).toInt() == 0) {
            ...
            QSqlQuery q(m_db);
            q.exec(QStringLiteral("INSERT INTO audit_log(username,role,action,target,details,created_at) VALUES('admin','admin','ĐIỀU KHIỂN RƠ-LE','190782','Bật quạt làm mát','%1')").arg(t1));
            q.exec(QStringLiteral("INSERT INTO audit_log(username,role,action,target,details,created_at) VALUES('hoangminh','user','CÀI ĐẶT CẤU HÌNH','190782','Cập nhật ngưỡng nhiệt độ & âm thanh','%1')").arg(t2));
            q.exec(QStringLiteral("INSERT INTO audit_log(username,role,action,target,details,created_at) VALUES('admin','admin','GHÉP NỐI THIẾT BỊ','190782','Gán trạm làm mát tự động','%1')").arg(t3));
        }
    }
                                             ...
                                             int limit, QString *error) const
{
    ...
    if (period == QStringLiteral("day")) {
        isFuture = (selDate.isValid() && selDate > today);
    } else if (period == QStringLiteral("month")) {
        isFuture = (selDate.isValid() && ((selDate.year() > today.year()) || (selDate.year() == today.year() && selDate.month() > today.month())));
    } else if (period == QStringLiteral("year")) {
        isFuture = (selDate.isValid() && (selDate.year() > today.year()));
    }
    ...
    if (period == QStringLiteral("month")) {
        ...
        while (query.next()) {
            ...
            const QDate d = QDate::fromString(dayStr, Qt::ISODate);
            if (d.isValid() && d > today)
                continue;

            QJsonObject metrics = QJsonDocument::fromJson(query.value(1).toByteArray()).object();
            if (metrics.contains(QStringLiteral("temperature_c"))) {
                const double t = metrics.value(QStringLiteral("temperature_c")).toDouble();
                if (t < 5.0 || t > 65.0)
                    metrics[QStringLiteral("temperature_c")] = qBound(18.0, t, 55.0);
            }
            if (metrics.contains(QStringLiteral("sound_vpp"))) {
                const double s = metrics.value(QStringLiteral("sound_vpp")).toDouble();
                if (s < 0.0 || s > 4.0)
                    metrics[QStringLiteral("sound_vpp")] = qBound(0.05, s, 2.5);
            }
    ...
    QSet<QString> metricKeys;
    while (query.next()) {
        const QString recStr = query.value(0).toString();
        const QDate d = QDate::fromString(recStr.left(10), Qt::ISODate);
        if (d.isValid() && d > today)
            continue;
        QJsonObject metrics = QJsonDocument::fromJson(query.value(1).toByteArray()).object();
        if (metrics.contains(QStringLiteral("temperature_c"))) {
        ...
        rows.append(QJsonObject{{"recorded_at", recStr},
                                {"metrics", metrics}});
    }
    ...
    while (aggregate.next()) {
        ++total;
        QJsonObject metrics = QJsonDocument::fromJson(aggregate.value(0).toByteArray()).object();
        if (metrics.contains(QStringLiteral("temperature_c"))) {
            const double t = metrics.value(QStringLiteral("temperature_c")).toDouble();
            if (t < 5.0 || t > 65.0)
                metrics[QStringLiteral("temperature_c")] = qBound(18.0, t, 55.0);
