...
void MqttDiscoveryService::processPublish(quint8 flags, const QByteArray &body)
{
    ...
    if (channel == QStringLiteral("status")) {
        ...
        // Giới hạn ghi log lịch sử tối thiểu 20 giây một mẫu để tránh dày đặc biểu đồ và tràn bộ nhớ DB
        if (nowMs - m_lastTelemetryLogMs.value(deviceId, 0) >= 20000) {
            QJsonObject filteredMetrics = metrics;
            if (metrics.contains(QStringLiteral("temperature_c"))) {
                double t = metrics.value(QStringLiteral("temperature_c")).toDouble();
                if (t < 5.0 || t > 65.0) {
                    t = qBound(18.0, t, 55.0);
                    filteredMetrics[QStringLiteral("temperature_c")] = t;
                }
            }
            if (metrics.contains(QStringLiteral("sound_vpp"))) {
                double s = metrics.value(QStringLiteral("sound_vpp")).toDouble();
                if (s < 0.0 || s > 4.0) {
                    s = qBound(0.05, s, 2.5);
                    filteredMetrics[QStringLiteral("sound_vpp")] = s;
                }
            }

            QString logError;
            if (!m_database->recordTelemetry(
                    deviceId, filteredMetrics,
